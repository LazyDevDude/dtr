        ...
        case 'check_date_exists':
            ...
            if ($check_date) {
                try {
                    $conn = getDBConnection();
                    if ($conn) {
                        $stmt = $conn->prepare("SELECT date, remarks, is_overtime, daily_hours, am_in, am_out, pm_in, pm_out FROM dtr_temp WHERE date = ?");
...
function saveToDBWithDate($date, $am_in, $am_out, $pm_in, $pm_out, $remarks) {
    try {
        ...
        // Insert or update
        $sql = "INSERT INTO dtr_temp (date, am_in, am_out, pm_in, pm_out, remarks, daily_hours, is_overtime) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?) 
        ...
        $stmt->bind_param("ssssssdi", $date, $am_in, $am_out, $pm_in, $pm_out, $remarks, $daily_hours, $is_overtime);
        $stmt->execute();
        $stmt->close();
        $conn->close();
    } catch (Exception $e) {
...
<head>
    ...
    <title>Jovagne's OJT DTR - NCIP APAYAO</title>
    ...
    <div class="container">
        <div class="main-card">
            ...
            <div class="p-4">
                ...
                <!-- Mini Calendar Widget (Only for current month) -->
                <?php if ($selectedMonth === date('Y-m')): ?>
                <div class="mini-calendar">
                    <h6 style="margin: 0 0 5px 0; color: var(--text-primary); font-weight: bold;">
                        <i class="bi bi-calendar3"></i> <?php echo date('F Y'); ?> Quick View
                    </h6>
                    ...
                    <div class="calendar-grid" id="mini-calendar-grid">
                        ...
                        for ($day = 1; $day <= $daysInMonth; $day++) {
                            ...
                            } elseif (in_array($day, $recordDays)) {
                                $classes[] = 'has-entry';
                                $title = 'Has Entry';
                            }
    ...
    <script>
        ...
        async function saveManualEntry() {
            ...
            if (!isRemarksOnlyEdit) {
                // If OT day is checked, skip time validation
                if (!isOTDay) {
                    // Validate at least one time field is filled
                    if (!amIn && !amOut && !pmIn && !pmOut) {
                        showToast('Please enter at least one time or check OT Day', 'error');
                        return;
                    }
                }
            } else {
        ...
        document.getElementById('manual-date').addEventListener('change', async function() {
            ...
            if (existingRecord.exists && !isFutureDate) {
                ...
                if (recordData.exists) {
                    ...
                    // Set and lock OT checkbox to match existing record
                    document.getElementById('ot-day-checkbox').checked = recordData.isOT;
                    document.getElementById('ot-day-checkbox').disabled = true;
                    
                    // Update time fields state based on OT status
                    toggleOTDay();
                }
                ...
                // Uncheck OT day when changing date
                document.getElementById('ot-day-checkbox').checked = false;
                toggleOTDay();
